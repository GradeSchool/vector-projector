...
/**
 * Commit an uploaded SVG file to the user's library.
 ...
 * 5. Server creates svg_files record
 */
export const commitFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.isBase) {
      ...
      path = `/base/svg/${fileId}.svg`;
    } else {
      // User files go in their directory
      const fileId = crypto.randomUUID();
      path = `/users/${identity.subject}/svg/${fileId}.svg`;
    }
    ...
    const fileId = await ctx.db.insert("svg_files", {
      userId: appUser._id,
      path,
...
/**
 * List all base SVG samples (for discovery mode).
